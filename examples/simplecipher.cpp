...
CipherAnalyzerPtr create_cipher() {
    ...
    AbstractBoxConstructor identity_constructor = []() { return make_shared<IdentityBox>(16); };

    map<string, AbstractBoxConstructor> constrs{{"Src", identity_constructor}, {"Sbox1", sbox_constructor},
                                                {"Sbox2", sbox_constructor},   {"Sbox3", sbox_constructor},
                                                {"Sbox4", sbox_constructor},   {"Pbox", pbox_constructor}};

    map<string, vector<NamedConnection>> conns{{"Src",
                                                {
                                                    {"Sbox1", {{0, 4}, {0, 4}}},
                                                    {"Sbox2", {{4, 4}, {0, 4}}},
                                                    {"Sbox3", {{8, 4}, {0, 4}}},
    ...
    for (int i = 0; i < round_cnt; i++) {
        rounds.push_back(make_shared<RoundFunction>("Src", "Pbox", constrs, conns));
    }
